...
// Supported languages
export const supportedLanguages = ['en', 'de', 'fr', 'zh', 'vi'] as const;
...
export const getLanguageFromUrl = (): SupportedLanguage => {
  ...
  const langMatch = path.match(/^\/(en|de|fr|zh|vi)(?:\/|$)/);
  if (
    langMatch &&
...
export const changeLanguage = (lang: SupportedLanguage): void => {
  ...
  let newPath: string;
  if (currentPath.match(/^\/(en|de|fr|zh|vi)\//)) {
    newPath = currentPath.replace(/^\/(en|de|fr|zh|vi)\//, `/${lang}/`);
  } else if (currentPath.match(/^\/(en|de|fr|zh|vi)$/)) {
    newPath = `/${lang}`;
  } else {
