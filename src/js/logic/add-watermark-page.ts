...
async function addWatermark() {
  ...
  try {
    ...
    if (watermarkType === 'text') {
      watermarkAsset = await pageState.pdfDoc.embedFont(
      ...
      if (imageFile.type === 'image/png') {
        watermarkAsset = await pageState.pdfDoc.embedPng(
          imageBytes as ArrayBuffer
        );
      } else if (imageFile.type === 'image/jpeg') {
        watermarkAsset = await pageState.pdfDoc.embedJpg(
          imageBytes as ArrayBuffer
        );
      } else {
        throw new Error(
