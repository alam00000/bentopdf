...
function initializePage(): void {
  ...
  const customTsaWrapper = getElement<HTMLDivElement>('custom-tsa-wrapper');
  ...
  if (tsaPreset && customTsaWrapper) {
    tsaPreset.addEventListener('change', () => {
      if (tsaPreset.value === 'custom') {
        customTsaWrapper.classList.remove('hidden');
      } else {
        customTsaWrapper.classList.add('hidden');
      }
    });
  }
...
function getTsaUrl(): string | null {
  ...
  if (tsaPreset.value === 'custom') {
    const customInput = getElement<HTMLInputElement>('custom-tsa-url');
    const customUrl = customInput?.value?.trim();
    if (!customUrl) {
      showAlert(
        'Missing TSA URL',
        'Please enter a custom TSA server URL.'
      );
      return null;
    }
    return customUrl;
