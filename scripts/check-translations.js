...
const NAMESPACE_FILES = ['common.json', 'tools.json'];
 ...
 */
function loadTranslation(lang, namespace = 'common.json') {
  const filePath = path.join(LOCALES_DIR, lang, namespace);
  ...
  try {
    ...
    console.error(
      `${colors.red}✗ Error parsing ${lang}/${namespace}:${colors.reset}`,
      error.message
    );
    return null;
  }
...
function main() {
  ...
  // Check each namespace file in every language
  for (const namespace of NAMESPACE_FILES) {
    // Load reference language (English)
    const refTranslation = loadTranslation(REFERENCE_LANG, namespace);
    if (!refTranslation) {
      console.error(
        `${colors.red}✗ Reference language (${REFERENCE_LANG}) not found for ${namespace}${colors.reset}`
      );
      process.exit(1);
    }
    ...
    console.log(
      `${colors.green}✓ Reference language (${REFERENCE_LANG}) ${namespace}: ${refKeys.length} keys${colors.reset}\n`
    );

    for (const lang of langsToCheck) {
      printHeader(`Checking: ${lang.toUpperCase()} (${namespace})`);

      const translation = loadTranslation(lang, namespace);
      if (!translation) {
        hasIssues = true;
        console.log(
          `${colors.red}✗ Missing ${lang}/${namespace}${colors.reset}\n`
        );
        continue;
      }
      ...
      if (missing.length === 0 && extra.length === 0) {
        console.log(
          `${colors.green} No missing or extra keys!${colors.reset}\n`
        );
      }
    }
  }
