...
const DEFAULT_CORS_PROXY_ORIGIN =
  'https://hiiirepdf-cors-proxy.hiiirepdf.workers.dev';
...
const directives = [
  ...
  `connect-src 'self' blob: https://fonts.gstatic.com ${connectOrigins.join(' ')}`.trim(),
  `object-src 'none'`,
  `base-uri 'self'`,
  `frame-src 'self' blob:`,
...
const docsDirectives = [
  ...
  `img-src 'self' data: blob: https:`,
  `font-src 'self' data: https://fonts.gstatic.com ${fontOrigins.join(' ')}`.trim(),
  `connect-src 'self' https://fonts.gstatic.com ${connectOrigins.join(' ')}`.trim(),
  `object-src 'none'`,
  `base-uri 'self'`,
  `frame-ancestors 'self'`,
  `form-action 'self'`,
